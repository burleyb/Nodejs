	...
	return {
		...
		checkLock: function(cron, runid, remainingTime, callback) {
			if (cron.time && (Date.now() - cron.time) > 2000) {
				...
				dynamodb.update(command, function(err, data) {
		...
		checkpoint: function(id, event, params, callback) {
			...
			dynamodb.update(cronCheckpointCommand, function(err, data) {
			...
			let get__cron = (d) => {
				...
				this.get(event.id || event.botId, {}, (err, data) => {
					...
					if (!cron.checkpoints) {
						...
						dynamodb.update(command, function(err) {
							d(err);
						});
					}
				});
