...
const { UpdateCommand, GetCommand, PutCommand, DeleteCommand, ScanCommand, QueryCommand, BatchGetCommand, BatchWriteCommand } = require('@aws-sdk/lib-dynamodb');
...
module.exports = function(configure, connectionConfig = {}) {
    ...
    var docClient = DynamoDBDocument.from(client, {
        marshallOptions: {
    ...
    let ddbLib = {
        ...
        query: function(params, configuration, stats) {
            ...
            var deferred = new Promise((resolve, reject) => {
                const command = config.method == "scan" ? new ScanCommand(params) : new QueryCommand(params);
                docClient.send(command)
                    .then(data => {
                        stats.mb++;
                        ...
                        config.progress(data, stats, function(shouldContinue) {
                            ...
                            if (shouldContinue && data.LastEvaluatedKey && stats.mb < config.mb && (config.count == null || stats.count < config.count)) {
                                ...
                                ddbLib.query(params, config, stats).then(function(innerData) {
        ...
        batchGet: function(params, opts, callback) {
            ...
            var chunker = chunk(function(items, done) {
                logger.log(`Batch getting: ${items.length}`);
                if (items.length > 0) {
                    ...
                    .then(data => {
                        ...
                        // Process each table's responses
                        // Object.entries(data.Responses).forEach(([tableName, tableResponses]) => {
                        //     logger.log(`found ${tableResponses.length} items in table ${tableName}`);
                        //     results = results.concat(tableResponses);
                        // });
                        results = data
            ...
            Object.values(params.RequestItems).forEach(table => {
        ...
        batchWrite: function(params, callback) {
            logger.log(`Sending records`);
            const command = new BatchWriteCommand(params);

            docClient.send(command)
                .then(data => {
                    if (data.UnprocessedItems && data.UnprocessedItems.length > 0) {
                        logger.log(`Unprocessed ${JSON.stringify(data)} records`);
                        callback("unprocessed records", data.UnprocessedItems);
                    } else {
                        callback(null, []);
                    }
                })
                .catch(err => {
                    logger.log(`All records failed`, err);
                    callback(err);
                });
        },
