	...
	var ls = {
		...
		fromLeo: (ID, queue, opts) => {
			...
			dynamodb.batchGet({
				...
				console.log("[leoEvent]", leoEvent, queue)
				...
				if (usingSnapshot || max_eid.localeCompare(start) > 0) {
					...
					async.whilst(() => {
						...
						logger.debug("totalCount: ", totalCount, "limit: ", opts.limit, "totalSize: ", totalSize, "size: ", opts.size, "hasMoreEvents: ", hasMoreEvents, "hasTime: ", hasTime, "hasLoops: ", hasLoops, "max_eid: ", max_eid, "start: ", start, "usingSnapshot: ", (usingSnapshot || max_eid.localeCompare(start)));
				...
				if (records.length === 0) {
					...
					for (let ndx = 0; ndx < all.length; ndx += 25) {
						...
						tasks.push(function(done) {
							...
							dynamodb.batchTableWrite(table, myRecords,
							function(err, data) {
								if (err) {
								...
								} else if (data.UnprocessedItems && table in data.UnprocessedItems && Object.keys(data.UnprocessedItems[table]).length !== 0) {
									//reset();
